...
class GuiButtonGroupProps(GuiBaseProps):
    ...
    colors: Optional[Tuple[Union[LiteralColor, Tuple[int, int, int], None], ...]]
    """(camtraj patch) Per-option color, same format as GuiButtonProps.color.
    None (the default, or a None entry) keeps that button's default styling."""
    sizes: Optional[Tuple[Optional[float], ...]]
    """(camtraj patch) Per-option relative width (flex-grow weight). None (the
    default, or a None entry) falls back to the original equal-width behavior."""
